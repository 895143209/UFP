...
def emit_section_file(data):
    ...
    lines += ["def _create_table_wall_section(*, tag, name, thickness, cover, concrete_layers, cover_concrete_tag, core_concrete_tag, cover_concrete_name=None, core_concrete_name=None, h_bar_dia=None, h_bar_spacing=None, v_bar_dia=None, v_bar_spacing=None):", "    h_t = ufp.rebar_equivalent_layer_thickness(h_bar_dia, h_bar_spacing) if h_bar_dia and h_bar_spacing else 0.0", "    v_t = ufp.rebar_equivalent_layer_thickness(v_bar_dia, v_bar_spacing) if v_bar_dia and v_bar_spacing else 0.0", "    core_total = thickness - 2.0 * cover - 2.0 * h_t - 2.0 * v_t", "    if core_total <= 0.0: raise ValueError(f'Layer thickness is not enough for {name}')", "    core_t = core_total / int(concrete_layers)", "    layers = [{'matTag': cover_concrete_tag, 'thickness': cover}]", "    if h_t: layers.append({'matTag': TagreinfH, 'thickness': h_t, 'rebar': True})", "    if v_t: layers.append({'matTag': TagreinfV, 'thickness': v_t, 'rebar': True})", "    for _ in range(int(concrete_layers)): layers.append({'matTag': core_concrete_tag, 'thickness': core_t})", "    if v_t: layers.append({'matTag': TagreinfV, 'thickness': v_t, 'rebar': True})", "    if h_t: layers.append({'matTag': TagreinfH, 'thickness': h_t, 'rebar': True})", "    layers.append({'matTag': cover_concrete_tag, 'thickness': cover})", "    section_data = {'tag': tag, 'name': name, 'thickness': thickness, 'material_properties': {cover_concrete_tag: {'name': cover_concrete_name or str(cover_concrete_tag), 'color': '#E2E764'}, core_concrete_tag: {'name': core_concrete_name or str(core_concrete_tag), 'color': '#C9D95B'}, TagreinfH: {'name': 'HRB400 Horizontal PlateRebar', 'color': '#d62728'}, TagreinfV: {'name': 'HRB400 Vertical PlateRebar', 'color': '#3327d6'}}, 'layers': layers}", "    return ufp.create_layershell(section_data, show=TABLE_SECTION_SHOW)", ""]
    ...
    for info in data["wall_sections"].values():
        ...
        lines += [f"        {var}_SecTag = {info['sec_tag']}", f"        _create_table_wall_section(tag={var}_SecTag, name={var!r}, thickness={py_num(row['h'])}, cover={py_num(row.get('cover') or 25)}, concrete_layers={to_int(row.get('wall_layers'), 2)}, cover_concrete_tag={material_expr(row.get('cover_mat'), 'TagWallC30Plate')}, core_concrete_tag={material_expr(row.get('core_mat'), 'TagWallC30Plate')}, cover_concrete_name={material_label(row.get('cover_mat') or 'TagWallC30Plate')!r}, core_concrete_name={material_label(row.get('core_mat') or 'TagWallC30Plate')!r}, h_bar_dia={fmt_number(hbar[0])}, h_bar_spacing={fmt_number(hbar[1])}, v_bar_dia={fmt_number(v[0])}, v_bar_spacing={fmt_number(v[1])})", ""]
